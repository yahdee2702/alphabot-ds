    ...
    ban: {
        ...
        runnable: (client, info, args) => {
            ...
            BanUser(user, (user0) => {
                ...
                console.log(`Cannot Ban user ${user0.tag} cause they are already banned.`)
                info.reply(`Cannot Ban user <@!${user0.id}> because they are already banned.`)
            })
        }
    },
    unban: {
        ...
        runnable: (client, info, args) => {
            ...
            UnBanUser(user, (user0) => {
                console.log(`Successfully Unbanned user ${user0.tag}.`)
                info.reply(`Successfully Unbanned user <@!${user0.id}>.`)
            }, (user0) => {
                console.log(`Cannot Unban user ${user0.tag} because they are already unbanned.`)
                info.reply(`Cannot Unban user <@!${user0.id}> because they are already unbanned.`)
            })
        }
    }
}